...
(function ($) {
  ...
  /*----------------------------- Site Loader & Popup --------------------*/
  const generateMainNavItem = (category) => {
    return `<ul class="d-block">
        <li class="menu_title">
  ...
  const generateSideNavItem = (category) => {
    return `<li>
        <a href="javascript:void(0)">${category.name}</a>
        <ul class="sub-menu">
          ${category.subCategories
  ...
  const generateFullSideNavItem = (category) => {
    return `<div class="ec-sb-block-content">
              <ul>
                <li>
                      ...
                      .map(
                        ...
                        <li>
                          <div class="ec-sidebar-sub-item">
                            <a href="/category.html?main=${category.name}&sub=${subCategory}">
                              ${subCategory}
                            </a>
                          </div>
                        </li>`
  ...
  const displayNavCategories = async () => {
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      $("#nav-category-container").append(category);
      $("#categories-mobile-sub-menu").append(mobileCategory);
      $("#full-category-list").append(categoryFullList);
    }
  ...
  const displayUserMenu = () => {
    ...
    $("#user-dropdown").append($.parseHTML(generateUserMenu(user)));
  ...
  $(window).load(async function () {
    try {
      ...
      if (
        !pathname.includes("category.html") &&
        !pathname.includes("index.html") &&
        pathname !== "/" &&
        !pathname.includes("product.html") &&
        !pathname.includes("wishlist.html")
      ) {
  ...
  $(".ec-change-mode .ec-mode-switch").click(function (e) {
    e.preventDefault();
    var $link = $("<link>", {
      rel: "stylesheet",
      href: "assets/css/dark.css",
...
const getCartItems = async () => {
  ...
  try {
    ...
    jsonResponse.Product.forEach((product) => {
      ...
      $("#side-cart-items").append($.parseHTML(generateSideCartItems(product)));
    });
  ...
  try {
    ...
    if (type === "Bespoke") {
      rawResponse = await fetch(
        `${API_URL}/order/bespoke/wishlist/add/${slug}`,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
      ...
      rawResponse = await fetch(
        `${API_URL}/order/product/wishlist/add/${slug}`,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
...
const addToCart = async (slug, quantity, type) => {
  const userObj = JSON.parse(localStorage.getItem("user"));
  try {
    if (!userObj) {
      window.location.href = `/login.html?${new URLSearchParams({
    ...
    if (type === "Bespoke") {
      rawResponse = await fetch(
        `${API_URL}/order/add_to_cart/bespoke/${slug}`,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
      ...
      rawResponse = await fetch(
        `${API_URL}/order/add_to_cart/product/${slug}`,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
...
const generateProductElement = (product) => {
  return `<div
  ...
  <div class="ec-product-inner">
    <div class="ec-pro-image-outer">
      <div class="ec-pro-image">
        <a href="product.html?item=${product.slug}" class="image">
          <img
            class="main-image"
            src="${product.image}"
            alt="${product.title}"
        ...
        <div class="ec-pro-actions">
          ...
          <a class="ec-btn-group wishlist" title="Wishlist" onclick="addToWishlist('${
            product.slug
          }','${product.type}')"
            ><img
    ...
    <div class="ec-pro-content">
      ...
      <div class="ec-pro-list-desc" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
        ${product.description}
      </div>
      <span class="ec-price">
...
const generateRatingItem = (filled) => {
  if (filled) return `<i class="ecicon eci-star fill"></i>`;

  return `<i class="ecicon eci-star-o"></i>`;
};
...
const generateUserMenu = (loggedIn) => {
  ...
  return `<ul class="dropdown-menu dropdown-menu-right">
    <li>
      <a class="dropdown-item" href="user/user-profile.html">Profile</a>
    </li>
    <li>
...
const generateSideCartItems = (product) => {
  return `<li>
    <a href="product.html?item=${product.slug}" class="sidekka_pro_img"
